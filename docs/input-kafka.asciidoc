...
* `[@metadata][kafka][serialzed_key_size]`: Integer size of serialized key in bytes, or `-1` if the record does not have a key.
* `[@metadata][kafka][serialzed_value_size]`: Integer size of serialized value in bytes, or `-1` if the record does not have a value.
